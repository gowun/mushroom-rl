...
from mushroom_rl.utils.torch import get_weights, set_weights, zero_grad, update_optimizer_parameters, cuda_device
...
class TorchApproximator(Serializable):
    ...
    def _compute_batch_loss(self, batch, use_weights, kwargs):
        if use_weights:
            ...
            weights = cuda_device(weights, self._use_cuda)
            #if self._use_cuda:
            #    weights = weights.cuda()
            batch = batch[:-1]

        torch_args = [cuda_device(torch.from_numpy(x), self._use_cuda) for x in batch]
        ...
        y = [cuda_device(y_i, self._use_cuda) for y_i in y]
    ...
    def diff(self, *args, **kwargs):
        ...
        """
        torch_args = [cuda_device(torch.from_numpy(np.atleast_2d(x)), self._use_cuda)
                      for x in args]
